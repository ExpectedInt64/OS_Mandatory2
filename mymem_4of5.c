...
void myfree(void* block)
{
	...
	cPtr = head;
	while(1){
		//We find the block we want to free
		if(cPtr->ptr == block){
			cPtr->alloc = false;
			//Check if block is adjacent to antoher block. If true then merge
			if(cPtr->last != NULL && cPtr->last->alloc == false){
				cPtr->last->size += cPtr->size;
				cPtr->last->next = cPtr->next;
				free(cPtr);
			}
			break;
